import { mostrarToast } from "../funcionesGaleriaMarcadoras/funcionesGaleria.js";

/*
    Setea los eventos para cerrar y abrir la pantalla donde se va a mostrar el carrito
*/
function setearEstadoCarrito(carrito, arrayMarcadoras) {
    ...
    agregarEventoVaciarCarrito(botonVaciarCarrito, carrito, arrayMarcadoras);
    let botonFinalizarCompra = document.getElementById("finalizar__compra");
    agregarEventoFinalizarCompra(botonFinalizarCompra, carrito, arrayMarcadoras);
}
/*
    Agrega al boton que vacia el carrito, su respectivo evento para cumplir con esta funcionalidad.
 */
function agregarEventoVaciarCarrito(botonVaciarCarrito, carrito, arrayMarcadoras) {
        ...
        if (carrito.length > 0) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    resetearBotonesCompra(arrayMarcadoras);
                    carrito.length = 0;
...
*/
function agregarEventoFinalizarCompra(botonFinalizarCompra, carrito, arrayMarcadoras) {
...
function crearProductoEnCarrito(producto, carrito) {
    ...
    divMarcadora.className = "producto__div";
    divMarcadora.setAttribute("id", producto.nombre);
    divMarcadora.innerHTML = `
    ...
    `;
    listaProductos.appendChild(divMarcadora);
    let botonEliminar = document.getElementById(`${producto.id}_eliminar`);

    botonEliminar.onclick = () => {
        let indiceBuscado = carrito.findIndex(Marcadora => Marcadora.nombre == producto.nombre);
...
function resetearBotonesCompra(arrayMarcadoras) {
    for (let Marcadora of arrayMarcadoras) {
        let boton = document.getElementById(`${Marcadora.id}_boton`);
        boton.innerText = "AGREGAR AL CARRITO";
        document.getElementById((Marcadora.id)).append(boton);
    }
}
/*
    Actualiza el boton de un Marcadora, de "AGREGAR AL CARRITO" a "EN EL CARRITO".
*/
function actualizarBotonCompra(Marcadoraseleccionado, carrito) {
    let idBuscado = `${Marcadoraseleccionado.id}_boton`;
    let botonBuscado = document.getElementById(idBuscado);
    if (carrito.some(Marcadora => Marcadoraseleccionado.nombre == Marcadora.nombre)) {
